...
def train(args, model, train_features, benchmarks, logger, checkpoints_dir):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for tag, features in benchmarks:
            ...
            epoch_output[tag + "_f1"] = output[tag + "_f1"]
        logger.info(str(epoch_output))
        result_output[epoch] = epoch_output
        
        epoch_checkpoints_dir = os.path.join(checkpoints_dir, 'epoch_'+str(epoch))
        os.makedirs(epoch_checkpoints_dir, exist_ok=True)
        torch.save(model, epoch_checkpoints_dir + '/model.pt')

    best_metric_epoch, best_metric_res = find_best_performance(result_output)
    
    logger.info('\n\nBEST EPOCH: ' + str(best_metric_epoch))
    logger.info('\n\nBEST RESULT: ')
    logger.info(json.dumps(best_metric_res, indent=True))
...
def main():
    ...
    # pdb.set_trace()
    logger = loadLogger(checkpoints_dir + '/logs')
    ...
    dev_file = os.path.join(args.data_test_dir, "dev.json")
    ...
    train(args, model, train_features, benchmarks, logger, checkpoints_dir)
